...
class Sor {
    ...
    async getParams() {
        var blockNames = ["GenParams", "SupParams", "FxdParams", "DataPts", "DataPts", "KeyEvents", "Cksum"];
        for (var bname in this.result['blocks']) {
            if (blockNames.includes(bname)) {
                let lowName = bname.toLowerCase();
                this.result.lowName = await this.reader[lowName](this.result, bname);
                break; // remove this!!!!
...
class Reader {
    ...
    async parseUnit(block) {
        for (const unit of block.units) {
            try {
                if (unit.type == "String") {
                    let result = await this.bf.readString(unit.length);
                    unit.result = result;
                }
    ...
    async genparams(r_result, blockname) {
        await this.checkBlock(r_result, blockname);
        let genparam = new Genparam(blockname);
        await this.parseUnit(genparam);
        console.log(genparam);

        return genparam;

    }
    async checkBlock(r_result, blockname) {
